...
class PDownstreamEvaluator(DownstreamEvaluator):
    ...
    def start_task(self, global_model):
        ...
        # th = 0.009  # RA
        th = self.thresholding(global_model)

        if self.global_:
            logging.info("use global_detection")
            self.global_detection(global_model)
        else:
            logging.info("use object_localization")
    ...
    def compute_residual(self, x_rec, x):
        ...
        # x_rec = median_filter(x_rec, size=(5, 5, 5))
        # x = median_filter(x, size=(5, 5, 5))
        x_rescale = exposure.equalize_adapthist(x.cpu().detach().numpy())
        x_rec_rescale = exposure.equalize_adapthist(x_rec.cpu().detach().numpy())
        saliency2 = self.get_saliency(torch.Tensor(x_rec_rescale).to(x_rec.device),
                ...
                for i in range(len(x)):
                    ...
                    if int(count) % 3 == 0 or int(count) in [67]:
                        ...
                        if 'embeddings' in x_rec_dict.keys():
                            ...
                            titles = ['Input', 'Rec', 'Res', 'SAl ' + str(np.max(saliency_i)),
                                      'Combined ' + str(np.max(x_res_i))]
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            for idx, data in enumerate(dataset):
                ...
                for i in range(len(x)):
                    ...
                    cnts_gt = cv2.findContours((mask_ * 255).astype(np.uint8), cv2.RETR_EXTERNAL,
