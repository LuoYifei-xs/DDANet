...
from model_zoo.D2S2 import FCSA, MSSA
...
class ResidualBlockWithFCSA(nn.Module):
    def __init__(self, inc=64, outc=64, groups=1, scale=1.0):
        super(ResidualBlockWithFCSA, self).__init__()
...
class ResidualBlockWithMSSA(nn.Module):
    """
    ...
    def __init__(self, inc=64, outc=64, groups=1, scale=1.0):
        super(ResidualBlockWithMSSA, self).__init__()
    ...
    def forward(self, x):
        if self.conv_expand is not None:
            ...
            identity_data = x
        output = self.relu1(self.bn1(self.conv1(x)))
        output = self.mssa(output)
        output = self.conv2(output)
        output = self.bn2(output)
        output = self.relu2(torch.add(output, identity_data))
        return output

class DM_LKF(nn.Module):
    def __init__(self, dim, out_channels) -> None:
        super().__init__()
        ...
        self.out_conv = nn.Conv2d(dim, out_channels, kernel_size=1, padding=0, stride=1)
...
class Encoder(nn.Module):
    def __init__(self, cdim=3, zdim=512, channels=(64, 128, 256, 512, 512, 512), image_size=256, conditional=False,
        ...
        self.main = nn.Sequential(
            ...
            # DynamicConvolution(in_channels=cdim, out_channels=cc),
            nn.BatchNorm2d(cc),
            nn.LeakyReLU(0.2),
            nn.AvgPool2d(2),
        )
        ...
        for ch in channels[1:]:
            if ch == 512 and sz == 8:
                self.main.add_module('res_in_{}'.format(sz), ResidualBlockWithFCSA(cc, ch, scale=1.0))
            elif ch == 128 and sz == 64:
                self.main.add_module('res_in_{}'.format(sz), ResidualBlockWithMSSA(cc, ch, scale=1.0))
            elif ch == 256 and sz == 32:
                self.main.add_module('res_in_{}'.format(sz), ResidualBlockWithMSSA(cc, ch, scale=1.0))
            else:
        ...
        self.main.add_module('res_in_{}'.format(sz), ResidualBlockWithFCSA(cc, cc, scale=1.0))
        ...
        if self.conditional:
            self.bottle_neck = DM_LKF(512, 512)
...
class Decoder(nn.Module):
    def __init__(self, cdim=3, zdim=512, channels=(64, 128, 256, 512, 512, 512), image_size=256, conditional=False,
        ...
        for ch in channels[::-1]:
            if ch == 512 and sz == 4:
                self.main.add_module('res_in_{}'.format(sz), ResidualBlockWithFCSA(cc, ch, scale=1.0))
            elif ch == 512 and sz == 8:
                self.main.add_module('res_in_{}'.format(sz), ResidualBlockWithFCSA(cc, ch, scale=1.0))
            elif ch == 128 and sz == 32:
                self.main.add_module('res_in_{}'.format(sz), ResidualBlockWithMSSA(cc, ch, scale=1.0))
            elif ch == 64 and sz == 64:
                self.main.add_module('res_in_{}'.format(sz), ResidualBlockWithMSSA(cc, ch, scale=1.0))
            else:
                self.main.add_module('res_in_{}'.format(sz), ResidualBlock(cc, ch, scale=1.0))
