...
import sys
sys.path.insert(0, '../DDANet')
from dl_utils.config_utils import *
import warnings
import os

os.environ["NUMEXPR_MAX_THREADS"] = "12"  # 设置为您希望的线程数，例如12
...
class Main(object):
    ...
    def setup_experiment(self):
        ...
        if configurator.dl_config['experiment']['task'] == 'train':
            ...
            logging.info("---------------------start_evaluations------------------------- ")
...
def add_args(parser):
    ...
    parser.add_argument('--config_path', type=str, default=r'E:\Project\Detection\DDANet\projects\25_DDANet\configs\fast_mri\DDANet.yaml', metavar='C',
...
if __name__ == "__main__":
    arg_parser = add_args(argparse.ArgumentParser(description='DDANet'))
    ...
    config_file = None
    logging.info(
        '------------------------------- DEEP LEARNING FRAMEWORK *DDANet*  -------------------------------')
    try:
        stream_file = open(args.config_path, 'r')
        config_file = yaml.load(stream_file, Loader=yaml.FullLoader)
        logging.info('[main] Success: Loaded configuration file at: {}'.format(args.config_path))
    except:
        logging.error('[main] ERROR: Invalid configuration file at: {}, exiting...'.format(args.config_path))
        exit()
