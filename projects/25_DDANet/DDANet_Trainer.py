...
import logging
from model_zoo.DDANet import *
from model_zoo.DDANet import reparameterize, calc_kl, calc_reconstruction_loss
    ...
    def train(self, model_state=None, opt_state=None, start_epoch=0):
        ...
        for epoch in range(self.training_params['nr_epochs']):
            ...
            for data in self.train_ds:
                ...
                expelbo_fake = (
                            -2 * self.scale * (self.beta_rec * loss_rec_fake_e + self.beta_neg * kl_fake)).exp().mean()
                ...
                lossD = self.scale * (loss_rec * self.beta_rec + (lossD_rec_kl + lossD_fake_kl) * 0.5 * self.beta_kl + self.gamma_r * 0.5 * self.beta_rec * (loss_rec_rec + loss_fake_rec))
            ...
            if epoch % 10 == 0 or epoch <= 20:
                ...
                fake_ = fake[0].cpu().detach().numpy()
                rec_fake_ = rec_fake[0].cpu().detach().numpy()
                elements = [img, rec_, np.abs(rec_ - img), fake_, rec_fake_]
                v_maxs = [1, 1, 0.5, 1, 1]
        ...
        if task == 'Val':
            if epoch_val_loss < self.min_val_loss:
                ...
                torch.save({
                    ...
                    'optimizer_e_weights': opt_weights[0],
                    'optimizer_d_weights': opt_weights[1],
                    'epoch': epoch
                }, self.client_path + '/best_model_{}_{}.pt'.format(epoch, epoch_val_loss))
